...
local function return_server_ok_authenticated()
  return server_response('Authentication Successful!', 'You may close this window')
end

local function return_server_ok_not_authenticated()
  return server_response('Authentication Denied!', 'You will not be able to use SMM!')
end

local function return_server_bad_request_csrf()
...
]] .. return_server_bad_request_csrf()
...
]] .. return_server_ok_not_authenticated()
  else
    response = [[HTTP/1.1 200 OK
Content-Type: text/html

]] .. return_server_ok_authenticated()
  end
